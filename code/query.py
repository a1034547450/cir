...
# ---------------------- 4. 查询与相似搜索 ----------------------
def generate_single_query_embedding(query_input, model, processor, device):
...
# ---------------------- 4. 查询与相似搜索 ----------------------
def generate_batch_query_embedding(dataset, model, processor, device,batch_size=8):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader):
            ...
            for image,ref_caption in zip(batch["reference_image_path"], batch["relative_caption"]):
                messages.append([construct_ref_record(image,ref_caption)])
            # 预处理多模态输入
            ...
            batch_meta_list = batch["reference_name"]  
...
def search_similar_samples(query_emb, index, meta_info, top_k=5):
    # 确保查询嵌入是二维数组（n, hidden_dim）
    assert len(query_emb.shape) == 2, f"查询嵌入形状应为(n, hidden_dim)，当前为{query_emb.shape}"
    # 确保查询嵌入维度与索引维度一致
    assert query_emb.shape[1] == index.d, \
        f"查询嵌入维度({query_emb.shape[1]})与索引维度({index.d})不匹配"

    n_queries = query_emb.shape[0]  # 获取批量大小

    # ---------------------- 批量搜索 ----------------------
    # FAISS原生支持批量输入，返回：
    # distances: 形状 [n_queries, top_k]，每个元素是对应查询与匹配样本的距离
    # indices: 形状 [n_queries, top_k]，每个元素是匹配样本在索引中的位置
    ...
    for query_idx in range(n_queries):
        ...
        for rank_idx in range(top_k):
            dist = distances[query_idx][rank_idx]
            sample_idx = indices[query_idx][rank_idx]

            # 跳过无效索引（FAISS在样本不足时会返回-1）
            if sample_idx == -1 or sample_idx >= len(meta_info):
                continue

            # 整理结果：排名从1开始（FAISS返回的结果已按距离从小到大排序）
            single_query_results.append({
                "meta_info": meta_info[sample_idx],  # 匹配样本的元信息（含generated_text）
                "distance": float(dist),  # 归一化后L2距离越小，相似度越高
                "rank": rank_idx + 1  # 排名直接取top_k的顺序（第0位是最相似，rank=1）
            })
        # 将单个查询的结果加入总列表
        all_results.append(single_query_results)
...
if __name__ == '__main__':
    ...
    for single_dress in dress_types:
        ...
        qry_dataset = Subset(qry_dataset,list(range(100)))
        save_dir = f'/data/oss_bucket_0/lida/cir/store/{single_dress}'
        # print("\nLoading index...")
        faiss_index, meta_info = load_index_and_meta(save_dir)
        
        query_embeddings, query_meta = generate_batch_query_embedding(qry_dataset,model,processor,DEVICE, BATCH_SIZE)
        print(len(query_meta))
        all_results = search_similar_samples(query_embeddings,faiss_index,meta_info,top_k=20)
        print(len(all_results))
        format_results = [] 

        for single_q_meta, single_result in zip(query_meta,all_results):
            # print(single_q_meta)
            # print(single_result)
            
            single_q_meta['result']=single_result
            format_results.append(single_q_meta)
        
        file_name = os.path.join(save_dir,'result.json')
        with open(file_name,mode='w',encoding='utf-8') as wf:
            json.dump(format_results,wf,indent=4,ensure_ascii=False)
